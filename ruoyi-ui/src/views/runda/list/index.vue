<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="污染类别" prop="polluteCategory">
        <el-select
          v-model="queryParams.polluteCategory"
          placeholder="请选择污染类别"
          clearable
        >
          <el-option
            v-for="dict in dict.type.pollute_category"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    >
      ...
      <el-table-column label="污染类别" align="center" prop="polluteCategory">
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.pollute_category"
            :value="scope.row.polluteCategory"
          />
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="诱发事故物质" prop="polluteGoods">
          <el-input
            v-model="form.polluteGoods"
            placeholder="请输入诱发事故物质"
          />
        </el-form-item>
...
export default {
  name: "List",
  dicts: ["station_status", "pollute_category", "notice_user"],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        polluteCategory: [
          { required: true, message: "污染类别不能为空", trigger: "change" },
        ],
        monitorCategory: [
          { required: true, message: "监控类别不能为空", trigger: "blur" },
        ],
        addr: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除单位名录编号为"' + ids + '"的数据项？')
        .then(function () {
          return delList(ids);
        })
        .then(() => {
