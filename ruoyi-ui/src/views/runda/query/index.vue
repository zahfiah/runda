...
<script>
import { listQuery, getQuery, delQuery, addQuery, updateQuery } from "@/api/runda/query";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除数据查询编号为"' + ids + '"的数据项？').then(function() {
        return delQuery(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('runda/query/export', {
        ...this.queryParams
