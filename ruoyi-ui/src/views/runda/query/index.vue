...
export default {
  ...
  methods: {
    ...
    resetQuery() {
      ...
      this.queryParams.timeType = "date"; // 重置为默认的日期类型
      this.handleQuery();
    },
    ...
    handleQuery() {
      ...
      if (this.queryParams.timeType === "date") {
        this.queryParams.beginTime = this.queryParams.startDate;
        this.queryParams.endTime = this.queryParams.endDate;
      } else {
        this.queryParams.beginTime =
          this.queryParams.selectedDate + " " + this.queryParams.startHour;
        this.queryParams.endTime =
          this.queryParams.selectedDate + " " + this.queryParams.endHour;
      }
      this.getList();
    },
    getList() {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除数据查询编号为"' + ids + '"的数据项？')
        .then(function () {
          return delQuery(ids);
        })
        .then(() => {
