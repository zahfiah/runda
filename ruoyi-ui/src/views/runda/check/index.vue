<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="checkList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="站点_id" align="center" prop="stationId" />
...
<script>
import { listCheck, getCheck, delCheck, addCheck, updateCheck } from "@/api/runda/check";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除手动矫正编号为"' + ids + '"的数据项？').then(function() {
        return delCheck(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('runda/check/export', {
        ...this.queryParams
