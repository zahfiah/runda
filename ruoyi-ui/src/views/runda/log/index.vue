  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="类型" prop="type">
        ...
        >
          <el-option
            v-for="dict in dict.type.type_info"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    >
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.msg" :value="scope.row.status" />
        </template>
      </el-table-column>
      <el-table-column
        label="创建时间"
      ...
      <el-table-column
        label="设备修改数"
        align="center"
        prop="deviceUpdatecount"
      />
      <el-table-column label="类型" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.type_info" :value="scope.row.type" />
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="设备添加数" prop="deviceAddcount">
          <el-input
            v-model="form.deviceAddcount"
            placeholder="请输入设备添加数"
          />
        </el-form-item>
        ...
        <el-form-item label="设备修改数" prop="deviceUpdatecount">
          <el-input
            v-model="form.deviceUpdatecount"
            placeholder="请输入设备修改数"
          />
        </el-form-item>
        <el-form-item label="类型" prop="type">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        dataCount: [
          { required: true, message: "数据条数不能为空", trigger: "blur" },
        ],
        deviceAddcount: [
          { required: true, message: "设备添加数不能为空", trigger: "blur" },
        ],
        msg: [{ required: true, message: "日志信息不能为空", trigger: "blur" }],
        status: [
        ...
        deviceUpdatecount: [
          { required: true, message: "设备修改数不能为空", trigger: "blur" },
        ],
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除接口日志编号为"' + ids + '"的数据项？')
        .then(function () {
          return delLog(ids);
        })
        .then(() => {
